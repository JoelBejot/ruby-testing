...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min is 0 and max is 9' do
      it 'returns 4' do
        min = 0
        max = 9
        expect(game_guessing.make_guess(min, max)).to eq(4)
      end
    end
    ...
    context 'when min is 5 and max is 9' do
      it 'returns 7' do
        expect(game_guessing.make_guess(5, 9)).to eq(7)
      end
    end

    context 'when min is 8 and max is 9' do
      it 'returns 8' do
        expect(game_guessing.make_guess(8, 9)).to eq(8)
      end
    end

    context 'when min is 0 and max is 3' do
      it 'returns 1' do
        expect(game_guessing.make_guess(0, 3)).to eq(1)
      end
    end

    context 'when min and max both equal 3' do
      it 'returns 3' do
        expect(game_guessing.make_guess(3, 3)).to eq(3)
      end
    end
  end
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      let(:same_numbers) { double('random_number', value: 8) }
      subject(:is_game_over) { described_class.new(0, 9, same_numbers, 8) }

      it 'is game over' do
        expect(is_game_over).to be_game_over
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      let(:same_numbers) { double('random_number', value: 8) }
      subject(:is_game_over) { described_class.new(0, 9, same_numbers, 7) }

      it 'is not game over' do
        expect(is_game_over).not_to be_game_over
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      ...
      it 'updates min to the same value as max' do
        maximum = edge_cases.max
        minimum = edge_cases.min
