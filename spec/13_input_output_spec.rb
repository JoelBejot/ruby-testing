...
describe NumberGame do
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        ...
        string = 'yrmom'
        valid_input = '7'
        allow(game_loop).to receive(:player_input).and_return(letter, string, valid_input)
      end
  ...
  describe '#final_message' do
    ...
    context 'when count is 2-3' do
      subject(:game_two_or_three) { described_class.new(5, '5', 3) }
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        congrats_phrase = "Congratulations! You picked the random number in 3 guesses!\n"
        expect { game_two_or_three.final_message }.to output(congrats_phrase).to_stdout
      end
    end
    ...
    context 'when count is 4 and over' do
      subject(:game_more_than_four) { described_class.new(5, '5', 8) }
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        final_phrase = "That was hard. It took you 8 guesses!\n"
        expect { game_more_than_four.final_message }.to output(final_phrase).to_stdout
      end
    end
  end
end
